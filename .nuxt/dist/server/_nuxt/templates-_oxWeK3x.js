import { ref, watch, mergeProps, unref, useSSRContext, computed, withCtx, createTextVNode, toDisplayString, isRef } from "vue";
import { ssrRenderAttrs, ssrInterpolate, ssrRenderAttr, ssrIncludeBooleanAttr, ssrRenderList, ssrRenderClass, ssrRenderComponent, ssrRenderStyle } from "vue/server-renderer";
...
import OpenNext from "./open-next-BTngSjB-.js";
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(templateStore).templates && unref(templateStore).templates.length >= 0) {
        _push(`<div class="portfolio-hero__meta" data-v-9aae542a><span class="meta-count" data-v-9aae542a> Найдено: <strong data-v-9aae542a>${ssrInterpolate(unref(templateStore).total ?? unref(templateStore).templates.length)}</strong></span>`);
        if (unref(templateStore).loading) {
          _push(`<span class="meta-loading" data-v-9aae542a> · загружается…</span>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(templateStore).error) {
          _push(`<span class="meta-error" data-v-9aae542a> · Ошибка: ${ssrInterpolate(unref(templateStore).error)}</span>`);
        } else {
          _push(`<!---->`);
        }
        _push(`</div>`);
...
const _sfc_main$3 = {
  ...
  setup(__props, { emit: __emit }) {
    ...
    const inlineBuild = computed(() => {
      var _a, _b;
      const t = ((_a = props.template) == null ? void 0 : _a.inline_build) ?? ((_b = props.template) == null ? void 0 : _b.inlineBuild);
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (!unref(templateStore).loading && __props.activeFilter !== "all") {
        ...
        _push(`<span data-v-1d275d61><small class="portfolio-grid__loading-hint" data-v-1d275d61>в категории «${ssrInterpolate(((_b = filters.value.find((f) => f.id === __props.activeFilter)) == null ? void 0 : _b.name) || __props.activeFilter)}»</small></span>`);
      } else {
        _push(`<!---->`);
      }
      _push(`</h2></div>`);
...
//# sourceMappingURL=templates-_oxWeK3x.js.map
