    ...
    <div class="container app-header__container">
      ...
      <div class="app-header__top">
        ...
        <div class="app-header__actions">
          ...
          <div v-if="isAuthenticated && user" class="app-header__profile">
            <button class="app-header__profile-toggle" @click="toggleUserMenu">
          ...
          <button class="app-header__menu-toggle" @click="toggleMobileMenu">
            <span
              class="app-header__menu-icon"
              :class="{ 'app-header__menu-icon--active': isMobileMenuOpen }"
            ></span>
          </button>
        </div>
      </div>
...
<style scoped>
/* Стили оставлены как в оригинале, добавлены мелкие правки для аватара */
.app-header { position: sticky; top: 0; background-color: var(--white); box-shadow: 0 2px 15px rgba(0, 0, 0, 0.08); z-index: 1000; }
.app-header__container { display: flex; flex-direction: column; }
.app-header__top { display: flex; justify-content: space-between; align-items: center; padding: 12px 0; }
.app-header__bottom { border-top: 1px solid #e2e8f0; padding: 10px 0; background: #ffffff; display: flex; }
.app-header__logo { display: flex; align-items: center; text-decoration: none; font-weight: 800; font-size: 1.8rem; color: var(--primary); position: relative; }
.app-header__logo-badge { background: linear-gradient(135deg, var(--primary) 0%, var(--primary-dark) 100%); color: white; font-size: 0.8rem; font-weight: 700; padding: 2px 8px; border-radius: 20px; position: relative; top: -5px; left: 5px; }
.app-header__actions { display: flex; align-items: center; gap: 15px; }
.app-header__telegram { display: flex; align-items: center; gap: 6px; padding: 8px 15px; text-decoration: none }
.app-header__telegram-icon { height: 26px; }
.app-header__telegram-img { height: 26px; width: 28px; }
...
.app-header__menu-toggle { display: none; background: none; border: none; width: 32px; height: 32px; position: relative; cursor: pointer; }
.app-header__menu-icon { position: absolute; top: 50%; left: 0; width: 100%; height: 3px; background: var(--primary); border-radius: 2px; transition: all 0.3s ease; transform: translateY(-50%); }
.app-header__menu-icon::before, .app-header__menu-icon::after { content: ''; position: absolute; left: 0; width: 100%; height: 3px; background: var(--primary); border-radius: 2px; transition: all 0.3s ease; }
.app-header__menu-icon::before { top: -8px; }
.app-header__menu-icon::after { top: 8px; }
.app-header__menu-icon--active { background: transparent; }
.app-header__menu-icon--active::before { top: 0; transform: rotate(45deg); }
.app-header__menu-icon--active::after { top: 0; transform: rotate(-45deg); }
.app-nav__list { display: flex; gap: 25px; list-style: none; }
.app-nav__link { text-decoration: none; color: var(--dark); font-weight: 600; position: relative; padding: 8px 0; transition: color 0.2s ease; font-size: 1rem; }
.app-nav__link:hover, .app-nav__link--active { color: var(--primary); }
.app-nav__link--active::after { content: ""; position: absolute; bottom: 0; left: 0; width: 100%; height: 3px; background: var(--primary); border-radius: 2px; }
.app-nav--primary { flex: 1; margin: 0 30px; }
.app-nav--secondary .app-nav__list { justify-content: center; }
.app-nav--secondary .app-nav__link { font-weight: 500; font-size: 0.95rem; color: var(--gray); }
.app-nav--secondary .app-nav__link:hover, .app-nav--secondary .app-nav__link--active { color: var(--primary); }
.app-mobile-menu { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0, 0, 0, 0.7); z-index: 999; opacity: 0; visibility: hidden; transition: all 0.3s ease; }
.app-mobile-menu--active { opacity: 1; visibility: visible; }
.app-mobile-menu__content { position: absolute; top: 0; right: 0; bottom: 0; width: 320px; background: white; padding: 20px; transform: translateX(100%); transition: transform 0.3s ease; }
.app-mobile-menu--active .app-mobile-menu__content { transform: translateX(0); }
.app-mobile-nav__section { margin-bottom: 25px; }
.app-mobile-nav__title { font-size: 1.1rem; color: var(--gray); padding: 10px 0; margin-bottom: 10px; border-bottom: 1px solid #e2e8f0; }
.app-mobile-nav__list { list-style: none; }
.app-mobile-nav__item { margin-bottom: 5px; }
.app-mobile-nav__link { display: block; padding: 12px 15px; text-decoration: none; color: var(--dark); border-radius: 8px; transition: background 0.2s ease; }
.app-mobile-nav__link:hover, .app-mobile-nav__link--active { background: #f1f5f9; color: var(--primary); }
@media (max-width: 1024px) { .app-nav__list { gap: 15px; } .app-nav--primary { margin: 0 15px; } }
@media (max-width: 900px) { .app-nav--primary, .app-nav--secondary { display: none; } .app-header__menu-toggle { display: block; } .app-header__top { padding: 15px 0; } }
@media (max-width: 480px) { .app-header__telegram-text { display: none; } .app-header__telegram-icon { margin-right: 0; } .app-header__logo { font-size: 1.6rem; } .app-header__logo-badge { font-size: 0.7rem; padding: 1px 6px; } }
</style>
